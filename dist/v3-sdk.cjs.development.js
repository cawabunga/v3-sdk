...
var TickListDataProvider = /*#__PURE__*/function () {
  ...
  _proto.getTick = function getTick(tick) {
    return TickList.getTick(this.ticks, tick);
  };

  _proto.nextInitializedTickWithinOneWord = function nextInitializedTickWithinOneWord(tick, lte, tickSpacing) {
...
var Pool = /*#__PURE__*/function () {
   ...
   */
  _proto.getOutputAmount = function getOutputAmount(inputAmount, sqrtPriceLimitX96) {
    ...
    var _this$swap = this.swap(zeroForOne, inputAmount.quotient, sqrtPriceLimitX96),
        outputAmount = _this$swap.amountCalculated,
        sqrtRatioX96 = _this$swap.sqrtRatioX96,
        liquidity = _this$swap.liquidity,
        tickCurrent = _this$swap.tickCurrent;

    var outputToken = zeroForOne ? this.token1 : this.token0;
    return [sdkCore.CurrencyAmount.fromRawAmount(outputToken, JSBI.multiply(outputAmount, NEGATIVE_ONE)), new Pool(this.token0, this.token1, this.fee, sqrtRatioX96, liquidity, tickCurrent, this.tickDataProvider)];
  }
  /**
  ...
  _proto.getInputAmount = function getInputAmount(outputAmount, sqrtPriceLimitX96) {
    ...
    var _this$swap2 = this.swap(zeroForOne, JSBI.multiply(outputAmount.quotient, NEGATIVE_ONE), sqrtPriceLimitX96),
        inputAmount = _this$swap2.amountCalculated,
        sqrtRatioX96 = _this$swap2.sqrtRatioX96,
        liquidity = _this$swap2.liquidity,
        tickCurrent = _this$swap2.tickCurrent;

    var inputToken = zeroForOne ? this.token0 : this.token1;
    return [sdkCore.CurrencyAmount.fromRawAmount(inputToken, inputAmount), new Pool(this.token0, this.token1, this.fee, sqrtRatioX96, liquidity, tickCurrent, this.tickDataProvider)];
  }
  /**
   * Executes a swap
  ...
  _proto.swap = function swap(zeroForOne, amountSpecified, sqrtPriceLimitX96) {
    ...
    var exactInput = JSBI.greaterThanOrEqual(amountSpecified, ZERO); // keep track of swap state
    ...
    while (JSBI.notEqual(state.amountSpecifiedRemaining, ZERO) && state.sqrtPriceX96 != sqrtPriceLimitX96) {
      ...
      var _this$tickDataProvide = this.tickDataProvider.nextInitializedTickWithinOneWord(state.tick, zeroForOne, this.tickSpacing);

      step.tickNext = _this$tickDataProvide[0];
      step.initialized = _this$tickDataProvide[1];
      ...
      var _SwapMath$computeSwap = SwapMath.computeSwapStep(state.sqrtPriceX96, (zeroForOne ? JSBI.lessThan(step.sqrtPriceNextX96, sqrtPriceLimitX96) : JSBI.greaterThan(step.sqrtPriceNextX96, sqrtPriceLimitX96)) ? sqrtPriceLimitX96 : step.sqrtPriceNextX96, state.liquidity, state.amountSpecifiedRemaining, this.fee);
      ...
      if (JSBI.equal(state.sqrtPriceX96, step.sqrtPriceNextX96)) {
        ...
        if (step.initialized) {
          var liquidityNet = JSBI.BigInt(this.tickDataProvider.getTick(step.tickNext).liquidityNet); // if we're moving leftward, we interpret liquidityNet as the opposite sign
